...
pub fn get_session_cookies(domains: Option<Vec<&str>>, cookies_dir: PathBuf) -> Result<Vec<Cookie>, Box<dyn std::error::Error>> {
    let mut cookies: Vec<Cookie> = vec![];
    ...
    let windows = json.get("windows").ok_or("no windows in json")?.as_array().ok_or("windows are not array")?;
        ...
        if let Some(cookies_json) = may_cookies_json {
            ...
            if let Some(cookies_json) = cookies_json {
                for json_cookie in cookies_json {
                    let domain = json_cookie.get("host").and_then(|v| v.as_str()).unwrap_or("");
                    if !domains.is_none() && !utils::is_substring_in_vec(domains.to_owned(), domain) {
                        continue;
                    }
...
pub fn get_session_cookies_lz4(domains: Option<Vec<&str>>, cookies_dir: PathBuf) -> Result<Vec<Cookie>, Box<dyn std::error::Error>> {
    let mut cookies: Vec<Cookie> = vec![];    
...
pub fn create_cookie(json_cookie: &Value) -> Result<Cookie, Box<dyn std::error::Error>> {
    let host = json_cookie.get("host").and_then(|v| v.as_str()).unwrap_or("");
    let path = json_cookie.get("path").and_then(|v| v.as_str()).unwrap_or("");
    let secure = json_cookie.get("secure").and_then(|v| v.as_bool()).unwrap_or(false);
    let name = json_cookie.get("name").and_then(|v| v.as_str()).unwrap_or("");
    ...
    let expires = json_cookie.get("expiry").and_then(|v| v.as_i64()).unwrap_or(0);
    let expires = utils::unix_timestamp_to_system_time(expires);
